			...
			<el-row>
			  ...
			  <!--轮播图-->
				...
				<el-col :span="4" class="con_log">
					<div>
            <img :src="userInfo.user_avatar" v-if="userInfo && userInfo.user_avatar"/>
						<img src="./img/no_login.jpg" v-else/>
					</div>
					<p>Hi~ 欢迎来到跨境商城</p>
					<p v-if="!userInfo || !userInfo.id"><router-link to="/login">登录</router-link><router-link to="/login">注册</router-link></p>
					<p v-else class="userIn">
            <a v-if="userInfo || userInfo.user_nickname">您好,{{ userInfo.user_nickname }}</a>
